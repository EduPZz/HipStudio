...
export class AlbumsService {
  ...
  async create(createAlbumDto: CreateAlbumDto) {
    const id_tipo = createAlbumDto.fk_TipoContrato;

    const canCreate = await this.prismaService.tipocontrato.findFirst({
      where: { ID_tipoContrato: id_tipo, can_albun: true },
    });
  ...
  async update(id: number, updateAlbumDto: UpdateAlbumDto) {
    const id_tipo = updateAlbumDto.fk_TipoContrato;
